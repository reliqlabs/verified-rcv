...
module rcv {
  ...
  type Ciphertext = str // Representing Vec<u8> for Quint's type system
  type DstackAttestation = str // Representing DstackAttestation structure

  type TallyResult = {
    winners: Vec[Addr],
    per_round_counts: Vec[Map[Addr, int]],
  ...
  type Message = {
    kind: MessageKind,
    sender: Addr,
    encrypted_preferences: Option[Ciphertext],
    tally_payload: Option[TallyResult],
    attestation_payload: Option[DstackAttestation]
  }
  ...
  const INITIAL_CANDIDATES: Vec[Addr]
  ...
  pure def is_tallying(): bool = env_block_time >= end_at and tally_result.is_none()
  pure def is_resolved(): bool = tally_result.is_some()

  // --- Pure Definitions (Helper for Invariants and Logic) ---

  // Simulates SHA-256 for binding, for type consistency
  pure def hash_placeholder(s: str): str = s.toSet().toStr().toUtf8()
  ...
  pure def image_registration_honest_at_current_state(): bool =
    dstack_kms_registry.get(ENCLAVE_ID_NAME).is_some() and
    let reg_entry = dstack_kms_registry.get(ENCLAVE_ID_NAME).get() {
  ...
  pure def verify_attestation(attestation: DstackAttestation, tally_body: TallyResult): bool =
    ...
    let expected_digest = hash_placeholder(CONTRACT_ADDR + canonical_serialization(tally_body)) {
  ...
  pure def Tally_spec_quint(raw_ballots: Map[Addr, Ciphertext], candidates_list: Vec[Addr], privkey: PrivKey): TallyResult =
    val num_tallied = raw_ballots.keys().size()
    val num_candidates = candidates_list.size()
    val non_voters_list = candidates_list.filter(c => not raw_ballots.keys().toSet().contains(c))
    val dummy_winners = if (num_tallied > 0) Vec(candidates_list.get(0)) else candidates_list // Default winner or all candidates
    val dummy_per_round = if (num_tallied > 0) Map(candidates_list.get(0) -> num_tallied) else Map()
    {
      winners: dummy_winners,
      per_round_counts: Vec(dummy_per_round),
      eliminated_by_round: Vec(Vec()),
  ...
  action init = all {
    ...
    fires_at_transition' = Set({kind: "Instantiate", sender: CONTRACT_ADDR, encrypted_preferences: none(), tally_payload: none(), attestation_payload: none()}),
  ...
  action submit_ballot_action(sender: Addr, encrypted_prefs: Ciphertext) = all {
    ...
    encrypted_prefs.size() > 0,
    ballots' = ballots.put(sender, encrypted_prefs),
    fires_at_transition' = fires_at_transition.union(Set({kind: "SubmitBallot", sender: sender, encrypted_preferences: some(encrypted_prefs), tally_payload: none(), attestation_payload: none()})),
    others
  }
  ...
  action close_and_tally_action = all {
    is_tallying(),
    fires_at_transition' = fires_at_transition.union(Set({kind: "CloseAndTally", sender: CONTRACT_ADDR, encrypted_preferences: none(), tally_payload: none(), attestation_payload: none()})),
    others
  }
  ...
  action publish_result_action(tally: TallyResult, attestation: DstackAttestation) = all {
    ...
    fires_at_transition' = fires_at_transition.union(Set({kind: "PublishResult", sender: CONTRACT_ADDR, encrypted_preferences: none(), tally_payload: some(tally), attestation_payload: some(attestation)})),
    others
  }
  ...
  action enclave_tally_action = all {
    is_tallying(),
    dstack_kms_derived_keys.get(CONTRACT_ADDR).is_some(),
    let privkey = dstack_kms_derived_keys.get(CONTRACT_ADDR).get() {
  ...
  val inv_B3_no_premature_tally: bool =
    (tally_result.is_none() and next.tally_result.is_some()) => env_block_time >= end_at
  ...
  val inv_B6_ballot_writer_is_the_ballot_voter: bool =
    let changed_ballots_keys = { k | k <- ballots.keys().union(next.ballots.keys()), ballots.get(k) != next.ballots.get(k) } {
      changed_ballots_keys.forAll(k =>
        ...
        let submit_msg = oneOf(fires_at_transition.filter(m => m.kind == "SubmitBallot" and m.sender == k)) {
          // (3) The new ballot value matches the message's encrypted_preferences
          next.ballots.get(k) == submit_msg.encrypted_preferences
        }
      )
    }
  ...
  val inv_B8_attestation_binds_tally: bool =
    (tally_result.is_none() and next.tally_result.is_some()) =>
      // Find the PublishResult message that fired (assuming uniqueness per transition for this purpose)
      fires_at_transition.exists(m => m.kind == "PublishResult" and m.tally_payload.is_some() and m.attestation_payload.is_some() and
        let tr = m.tally_payload.get() {
        let att = m.attestation_payload.get() {
...
- **S5 (terminality of resolution - handler-set property):** The intent notes this is a handler-set meta-property. Quint cannot directly analyze "only Block 6 writes." Instead, its direct behavioral consequence (`if tally_result.is_some() then next.tally_result == tally_result`) is encoded. This overlaps with `B1`, which explicitly models the temporal "monotone-once-set" behavior. The "Block 6's Requires include `tally_result.is_none()`" is enforced in `publish_result_action`'s guards.
