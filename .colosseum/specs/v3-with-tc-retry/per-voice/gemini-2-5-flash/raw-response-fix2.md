...
module rcv {
  ...
  action step = any {
    ...
    // Only allow instantiate if not yet instantiated
    (size(candidates_s) == 0) and instantiate(CANDIDATES, START_AT, END_AT),

    // Candidate submits ballot (must be in voting window)
    (isVoting()) and val sender_addr = oneOf(candidates_s), // Pick from *current* candidates_s
    ...
    (isTallying()) and close_and_tally(),
    ...
    (isTallying()) and val all_voters = ballots.keys().toList(),
