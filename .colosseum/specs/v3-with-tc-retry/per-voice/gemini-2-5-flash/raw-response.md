...
module rcv {
  ...
  var tally_result: Option[TallyResult] // None until published; immutable once set
  ...
  var submit_ballot_sender: Option[Addr]
  ...
  pure def isTallying(): bool = env_block_time >= end_at_s and tally_result.isNone()
  pure def isResolved(): bool = tally_result.isSome()

  // S1: non-empty candidates
  pure def S1_nonEmptyCandidates(): bool = size(candidates_s) >= 1

  // S2: distinct candidates
  pure def S2_distinctCandidates(): bool = Set(candidates_s).size() == size(candidates_s)
  ...
  pure def S6_winnerWellFormed(tally: TallyResult): bool =
    tally.winners.subset(Set(candidates_s)) and size(tally.winners) >= 1 and size(tally.winners) <= size(candidates_s)
  ...
  // S9: elimination monotonicity (from Block 6 Requires)
  pure def S9_eliminationMonotonicity_Full(tally: TallyResult): bool =
    tally.per_round_counts.zipWith(tally.eliminated_by_round, (round_counts, eliminated_in_round) => {
      // All candidates in `eliminated_in_round` must not be in `round_counts` for that specific round
      Set(eliminated_in_round).intersect(round_counts.keys()).size() == 0
    }).all(b => b)
    and (
      // For any round `i`, candidates eliminated *before* round `i` must not appear in `per_round_counts[i]`
      tally.per_round_counts.zipWithIndex((round_counts, i) => {
        val all_eliminated_before_i = tally.eliminated_by_round.take(i).flatten().toSet()
        all_eliminated_before_i.intersect(round_counts.keys()).size() == 0
      }).all(b => b)
    )
  ...
  pure def B6_wellFormedSetRelations(tally: TallyResult): bool =
    tally.non_voters.toSet() == Set(candidates_s).diff(ballots.keys())
    and tally.dropped_voters.subset(ballots.keys())
    and tally.dropped_voters.toSet().intersect(tally.non_voters.toSet()).size() == 0
  ...
  action instantiate(new_candidates: List[Addr], new_start_at: Timestamp, new_end_at: Timestamp): bool = all {
    ...
    S1_nonEmptyCandidates().with("candidates_s", new_candidates),
    S2_distinctCandidates().with("candidates_s", new_candidates),
  ...
  action advanceTime(new_time: Timestamp): bool = all {
    ...
    new_time >= env_block_time, // Non-decreasing (Block time monotonicity)
  ...
  action submit_ballot(sender: Addr, encrypted_prefs: Bytes): bool = all {
    ...
    submit_ballot_sender' = some(sender),
    // Requires:
    isVoting(),
  ...
  action publish_result(new_tally: TallyResult, attestation_blob: Bytes): bool = all {
    ...
    S9_eliminationMonotonicity_Full(new_tally), // Use the more robust full check
    B6_wellFormedSetRelations(new_tally),

    // Produces:
    tally_result' = some(new_tally),
    candidates_s' = candidates_s,
    start_at_s' = start_at_s,
    end_at_s' = end_at_s,
    enclave_pubkey_s' = enclave_pubkey_s,
  ...
  def B1_tallyResultMonotoneOnceSet(): bool =
    tally_result.isSome() => next.tally_result.isSome() and next.tally_result == tally_result
  ...
  def B3_noPrematureTally(): bool =
    tally_result.isNone() and next.tally_result.isSome() => env_block_time' >= end_at_s
  ...
  // This is implicitly covered by B1, as tally_result's value cannot change once set.
  // The 'publish_result' action's 'Requires' (isTallying()) ensures it only fires if tally_result is None.
  ...
  def B6_ballotWriterIsVoter(): bool =
    (ballots' != ballots) =>
      ...
      // No keys are removed
      ballots.keys().diff(ballots'.keys()).toSet().size() == 0 and
      (
        // Either a new key was added by the sender
        (ballots'.keys().diff(ballots.keys()).toSet() == Set(submit_ballot_sender.get())) or
        // Or an existing key was updated by the sender
        (ballots'.keys() == ballots.keys() and ballots.get(submit_ballot_sender.get()) != ballots'.get(submit_ballot_sender.get()))
      )
  ...
  // B8, B9, B10, B10_lean are cross-layer/meta-security and cannot be fully modeled in Quint.
  // Their classical-Prop shadows or key chain-side requirements are included in `publish_result` action's `Requires`.
  // The full implications are noted in design-notes.md.

  // --- Composite Invariant ---
  val all_invariants = all {
    // Structural Invariants
    S1_nonEmptyCandidates(),
    S2_distinctCandidates(),
    ...
    tally_result.isSome() => S6_winnerWellFormed(tally_result.get()),
    tally_result.isSome() => S7_tallyCountConservation(tally_result.get()),
    tally_result.isSome() => S8_perRoundCountConsistency(tally_result.get()),
    tally_result.isSome() => S9_eliminationMonotonicity_Full(tally_result.get()),
    tally_result.isSome() => B6_wellFormedSetRelations(tally_result.get()),
  ...
  action step = any {
    ...
    val sender_addr = oneOf(CANDIDATES),
    ...
    val mock_non_voters_list = candidates_s.filter(c => not(c.in(all_voters)))
    val num_non_voters = size(mock_non_voters_list)

    val possible_dropped_count = nondet(0.to(num_submitted_ballots))
    val mock_dropped_voters_list = all_voters.take(possible_dropped_count)

    val actual_tallied_count = num_submitted_ballots - possible_dropped_count
    
    // Ensure at least one winner for S6, if possible.
    val possible_winners = candidates_s.filter(c => not(c.in(mock_dropped_voters_list.toSet()))).filter(c => not(c.in(mock_non_voters_list.toSet())))
    ...
    val mock_tally_for_publish: TallyResult = {
      ...
      dropped_voters: mock_dropped_voters_list,
...
- **S1 (non-empty candidates):** `pure def S1_nonEmptyCandidates(): bool = size(candidates_s) >= 1`
- **S2 (distinct candidates):** `pure def S2_distinctCandidates(): bool = Set(candidates_s).size() == size(candidates_s)`
...
- **S5 (terminality of resolution, handler-set property):** The intent clarifies S5 as a handler-set property. The `tally_result` state variable is initialized to `none` and can only be set to `some` by `publish_result`. The `publish_result` action's `isTallying()` precondition ensures it only fires when `tally_result.isNone()`. Once `tally_result` is set, `isTallying()` becomes false, preventing further modification. The trajectory aspect ("once `Some`, always `Some` with the same value") is covered by **B1**. Thus, S5's implications are covered by action guards and B1.
...
- **B1 (tally_result monotone-once-set):** `def B1_tallyResultMonotoneOnceSet(): bool = tally_result.isSome() => next.tally_result.isSome() and next.tally_result == tally_result`. This uses Quint's `next` operator for temporal reasoning.
...
- **B3 (no premature tally):** `def B3_noPrematureTally(): bool = tally_result.isNone() and next.tally_result.isSome() => env_block_time' >= end_at_s`.
...
- **B5 (publish_result fires at most once):** This is stated as a derived corollary of B1 + Block 6. Its essence is captured by **B1** and the fact that `publish_result` is guarded by `isTallying()` (which includes `tally_result.isNone()`). It's explicitly tied to `B1_tallyResultMonotoneOnceSet()` in the spec.
...
- **B10 (tally-correctness) & B10_lean (Lean-internal image-IO obligation):** These are `cross-layer` and `off-chain` invariants respectively, discharged by the Lean proof and build pipeline. Quint cannot directly verify `tally = Tally_spec(...)`. Instead, the `publish_result` action *assumes* the provided `new_tally` is the correct `Tally_spec` output and enforces all the *well-formedness properties* (S6-S9, B6_wellFormedSetRelations) that `Tally_spec` is expected to guarantee for its output. The `dstack_kms_derived` and `enclave_input_fidelity` parts of B10 are not directly modeled.

#### Omitted Invariants and Justification

- **S5 (terminality of resolution, handler-set property):** While the spirit of S5 (at-most-once write to `tally_result`) is fully captured by action guards and B1, its statement as a purely static "handler-set property" from §3.1 is not directly translated as a `val` in `all_invariants` due to Quint's focus on state-based properties over a trace.
...
- **Ghost Variables for Action Attribution:** `submit_ballot_fired` and `submit_ballot_sender` are introduced to track if the `submit_ballot` action fired and by whom in the current step. This is necessary for `B6` to attribute ballot changes. These variables are reset (`false` or `none`) by any other action or in the subsequent step, ensuring only one action is attributed per transition. `ballots_at_end_at` is another ghost variable crucial for `B2`.
