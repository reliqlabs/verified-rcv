...
**Document version: 0.3.9** (SemVer per Colosseum methodology v0.4 candidate Ask N — see `colosseum/methodology-v0.4-candidates.md`). Version-bump classification:
...
- **`data[0..32]`** carries a SHA-256 digest whose preimage depends on the quote purpose:
  ...
  - **Publish quote** (issued by the enclave at tally completion): `data = SHA-256(canonical_serialization(contract_addr ‖ election_id ‖ tally_body))` per v0.3.8 M2 — unchanged from v0.3.8 except for residence: was lower 32 bytes of `user_data`; is now `ReportData[0..32]` in the gnark public_inputs blob.
...
- **Requires:**
  ...
  - `ReportData[0..32]` extracted from `public_inputs` equals `SHA-256(canonical_serialization(contract_addr ‖ election_id ‖ tally_body))` — see B8 clause (c)
...
- **Produces:** (equivalently: compute `Tally_spec(ballots, candidates, enclave_privkey)` per Section 2.5, then attest + submit)
  ...
  4. **Request TDX quote** from `tappd` (dstack's Trusted App Daemon) with `report_data = SHA-256(canonical_serialization(contract_addr ‖ election_id ‖ tally_body)) ‖ DST_VERIFIED_RCV_TALLY_V1_PADDED` (per §2.5 ReportData layout — publish quote form).
