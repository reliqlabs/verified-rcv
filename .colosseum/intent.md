...
**Document version: 0.3.14** (SemVer per Colosseum methodology v0.4 candidate Ask N — see `colosseum/methodology-v0.4-candidates.md`). Version-bump classification:
...
- v0.3.14 — 2026-05-27 MINOR: human-readable candidate names. Adds `candidate_names: Vec<String>` parallel to `candidates: Vec<Addr>` on Election state. Names are admin-supplied at CreateElection time, immutable thereafter (new B-clause B11). Names are bound by the publish attestation via a new `names_hash[32]` field in `canonical_serialization`, sitting between `ballots_hash` and `tally_body`. Field-spec discipline (v0.4 ask AI): each name ≤ 64 bytes UTF-8 (no NUL); `candidate_names.len() = candidates.len()`; no duplicate names (case-sensitive byte equality). `names_hash = SHA-256(DST_VERIFIED_RCV_NAMES_V1 ‖ u32_LE(candidates.len()) ‖ for i in 0..N: Borsh(candidate_names[i]))` in candidate-declaration order, where `DST_VERIFIED_RCV_NAMES_V1 = b"verified-rcv:names:v1"` (21 ASCII bytes). Parallel DST applied retroactively to `ballots_hash`: prefix `DST_VERIFIED_RCV_BALLOTS_V1 = b"verified-rcv:ballots:v1"` (23 ASCII bytes). New helper `compute_names_hash` (contract + enclave, cross-tested for byte-identity, parallel to `compute_ballots_hash` from v0.3.11). New error variants: `CandidateNamesLengthMismatch`, `CandidateNameEmpty`, `CandidateNameTooLong`, `CandidateNameInvalidUtf8`, `CandidateNameContainsNul`, `DuplicateCandidateName`. **B-clause B11** added to §3.2: `candidate_names` immutable post-CreateElection — no handler mutates the field after CreateElection completes (handlers that re-save the `Election` struct preserve the field byte-identically; the discipline is non-mutation, not non-write); the no-mutation discipline is encoded as a §6.7 contract-migration trust-boundary clause (introduction of any handler mutating `candidate_names` is a MAJOR-bump behavioral break). **B8(e) registration binding strengthened** (F1 from intent-adversarial fan-out): registration ReportData preimage extended from `SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id))` to `SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id) ‖ names_hash)`. Defense-in-depth: tamper with names between CreateElection and PublishResult is now detectable at CreateElection time, not only at PublishResult time. Parallels the v0.3.12 N22 triple-binding fix. **§8.7 link 7** annotation extended: input-fidelity now covers `(ballots_hash, names_hash)` tuple via DST-separated hashes. **§6.4 caller-contract scope** refreshed: names join the off-chain candidate-legitimacy attack surface; visual-confusability accepted as scope-bounded (chain enforces byte-distinctness only; UIs SHOULD warn on confusable-character detection). **§2.5 pairing clarification** (F4): `(addr_i, name_i)` pair binding is bound transitively — `ballots_hash` iterates in declaration order over `candidates` (binding address ordering), `tally_body` carries addresses keyed by candidate (binding address content), `names_hash` binds names in declaration order. The triple together is injective over the pair set; no separate `candidates_hash` field is needed. **§6.7 contract-migration discipline** (F3, new section): future contract upgrades MUST preserve the no-`UpdateNames`-handler invariant; introduction of any handler that *mutates* `Election.candidate_names` post-create is a behavioral break requiring MAJOR version bump and a fresh trust-boundary audit. Handlers that re-save the `Election` struct while preserving the `candidate_names` byte payload (e.g., `exec_submit_ballot` which increments `ballot_count` and writes the whole struct back) are compliant — the invariant is non-mutation, not non-write. The discipline is enforced structurally (absence of mutating code), not via a runtime lock-flag (which a determined attacker who can change handlers can also clear). **MINOR per the v0.3.0 rubric**: trust boundary narrowed (registration-time tamper detection added; DSTs added; structural migration discipline encoded); new symbols added (`candidate_names`, `names_hash`, `compute_names_hash`, `DST_VERIFIED_RCV_NAMES_V1`, `DST_VERIFIED_RCV_BALLOTS_V1`, B11, 6 error variants, §6.7 migration discipline); no invariant weakened; no behavior previously specified becomes unspecified. **Methodology trace (v0.4 paying for itself)**: this entry incorporates findings from the v0.3.14 intent-adversarial fan-out (`.colosseum/attacks/intent-v0.3.14-2026-05-27/synthesis.md`) — 4-voice cross-family panel via gateway (Claude / gemini-2-5-flash / nemotron-3-120b / gpt-oss-120b). Three of four voices independently surfaced the registration-names-binding asymmetry (F1, MAJOR); two surfaced the DST-separation gap (F2, MINOR); one surfaced the B11-structural-fragility (F3, MINOR); one surfaced the pairing-binding doc gap (F4, INFORMATIONAL). The author-self-review (inline lens application by the implementation agent) caught only the visual-confusability scope-bound; the cross-family panel caught three additional findings the author missed. **AB.1 discipline ("distinct agent from implementation author") empirically validated by this pass.**
- v0.3.13 — 2026-05-26 MINOR: citation/annotation bootstrap per v0.4 methodology asks AB.2 (clause-to-line discharge), AC (top-down Kani catalog), and AD (ledger-as-gate) — see `.colosseum/methodology-retrospective-2026-05-26.md`. The v0.3.12 validation pass identified that §3.2 B-clauses and §8.7 ledger links lacked structural annotation columns mapping each named obligation to its discharging code line + Kani harness (where applicable). This entry adds: (1) a `#### Discharge annotations` subsection at the end of §3.2 listing each B-clause (B1, B6, B8 a/b/c/d/e, B9, B10, B10_lean) with a `code: file:line` field pointing into `crates/contract/src/contract.rs`, `crates/contract/src/verification.rs`, `crates/contract/src/state.rs`, `crates/enclave/src/attestation.rs`, or `specs/*.lean` — with explicit `off-chain`/`axiom`/`upstream` reasons where no direct chain-side line discharges (B9 meta-security summands, B10 cross-layer composition, B10_lean Lean theorem); plus a `kani:` field naming the harness (already_resolved_enforced, b1_tally_result_present_after_publish, already_voted_enforced, s4_ballot_keys_subset_of_candidates, s6/s7/s8/s9 harnesses) or an explicit skip reason for each. (2) A `#### Per-link discharge annotations` subsection at the end of §8.7 covering all 7 composition links + the composition theorem (link 8), each with `code:` + `kani:` annotations following the same discipline. **No invariant content changes; no behavior changes; no trust boundary movement.** MINOR per the v0.3.0 rubric: discipline added (the citation/annotation structure is a new methodology obligation that the document now satisfies), no invariant weakened. Acceptance: every named B-clause and every §8.7 link now resolves to either a concrete `file:line` citation or an explicit `off-chain`/`axiom`/`upstream` annotation with reason; no empty annotations. Future intent revisions must keep these annotations current as discharge sites move (drift surfaces as missing-line CI failures per ask AD).
- v0.3.12 — 2026-05-26 MINOR + PATCH: third-round audit re-review (N17 Major + N18/N19/N20 Minor + N21/N22 Informational). **N17 (Major DoS)**: an attacker could sequence `propose → create_election → wait → finalize` to swap the registry mid-flight and DoS every PublishResult with `AttestationMeasurementMismatch`. Closed with TWO gates: (1) `exec_create_election` refuses when `PENDING_REGISTRY.may_load` is `Some` (admin must finalize or cancel first); (2) `exec_finalize_registry_update` refuses during `Voting` / `Tallying` phase as defense-in-depth (unreachable under (1) but cheap to add). New error variants: `PendingRegistryUpdateBlocksCreateElection`, `FinalizeDuringActiveElection`. **N18 (CI broken — cargo audit)**: `cargo audit --deny warnings` failed on `RUSTSEC-2024-0436` (paste 1.0.15 unmaintained warning, transitive via cosmwasm-crypto, not removable at this layer). Added `--ignore RUSTSEC-2024-0436`; track upstream cosmwasm fix. **N19 (CI broken — false-positive regex)**: the symbol-grep regex `mock_attestation|verification\b` false-positived on the literal "verification-key" inside `validate_registry`'s user-facing error string. Two-pronged fix: (a) narrow the regex to whole-word `mock_attestation` only — that's an actual feature-mangled-symbol pattern, not arbitrary text; (b) rename "verification-key" → "vkey identifier" in the error string so even loose regexes don't trip. **N20 (CI broken — cosmwasm-check rejects raw cargo build)**: raw `cargo build --release --target wasm32-unknown-unknown` on Rust 1.82+ emits wasm features (bulk-memory, reference-types, sign-ext) that cosmwasm-vm 3.x rejects at static deserialization, regardless of `-C target-feature=-bulk-memory,...` flags (compiler-builtins emits these ops unconditionally). The auditor flagged this as "the cosmwasm-check gate is correct; the underlying issue isn't fixed." **Fix**: production wasm is now built via the `cosmwasm/optimizer:0.17.0` Docker image (the standard CosmWasm deploy pipeline), which handles post-build wasm-feature lowering correctly. CI's `cosmwasm-check` step now runs against the optimizer-produced `artifacts/<crate>.wasm`, not the raw cargo output. `.cargo/config.toml` still carries the target-feature rustflags for *local dev sanity* (gets `cargo build` closer to the optimizer output), but is documented as not a production replacement. **N21 (Informational — partial archive)**: `HISTORICAL_TALLIES` archived `TallyResult` but not the `Election` metadata (title, start_at, end_at, candidates, enclave_pubkey). Added `HISTORICAL_ELECTIONS: Map<u64, Election>` and `QueryMsg::HistoricalElection { election_id }` so consumers fully recover superseded elections. **N22 (Informational — registration quote replay)**: the v0.3.11 registration ReportData hashed only `SHA-256(enclave_pubkey)`. An admin who controlled the same pubkey across elections (leaked privkey scenario) could replay an old quote for a new election, exposing the new election's ballots to off-chain decryption. The earlier v0.3.10 commit incorrectly cited "N2 timelock judged sufficient" — N2 prevents registry rotation, NOT registration quote replay. **Closed**: registration ReportData now binds `(enclave_pubkey, contract_addr, election_id)` instead of pubkey-only. Preimage: `SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id))`. Chain reads `next_id = ELECTION_COUNTER + 1` at PublishResult time and includes it in the expected hash. Runtime's `produce_registration_artifacts` now takes `(contract_addr, election_id)` and threads them into `build_registration_report_data`. Replay across elections fails `AttestationPubkeyBindingMismatch`. Diagnostic: a quote in the v0.3.11 pubkey-only form rejects with explicit `RegistrationQuoteWrongElection` (back-compat surfacing). **MINOR per the v0.3.0 rubric**: trust boundary narrowed (N17 + N22 close two practical attack surfaces; the registration binding moves from one-axis to three-axis cryptographic commitment). New symbols added: `HISTORICAL_ELECTIONS`, `HistoricalElection` query, `PendingRegistryUpdateBlocksCreateElection`, `FinalizeDuringActiveElection`, `RegistrationQuoteWrongElection` error variants; `build_registration_report_data` gains `contract_addr` + `election_id` parameters. No invariant weakened. **B8(e) reformulation**: the v0.3.9 form ("`enclave_pubkey` registered at `CreateElection` matches `public_inputs.ReportData[0..32]` of the registration quote") becomes "`enclave_pubkey` + `(contract_addr, election_id)` triple binds to `ReportData[0..32]`" — strictly stronger (replay-prevention added). Note: the v0.3.10 commit's deferral claim for N13 (now N22) was explicitly INCOMPLETE per the auditor; this entry is the correction.
...
- `candidate_names: Vec<String>` (v0.3.14) — display names parallel-indexed to `candidates`; declared at CreateElection, immutable. Length equals `candidates.len()`. Each name is ≤ 64 bytes UTF-8 (no embedded NUL). No two names in the same election are byte-equal (duplicate-name check at CreateElection). Names are **off-chain trust** in the §6.4 sense — they are admin-supplied display labels, not a cryptographic binding to voter intent.
...
- `canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ ballots_hash ‖ names_hash ‖ tally_body)` denotes `borsh_bytes(contract_addr) ‖ borsh_bytes(chain_id) ‖ u64_LE(election_id) ‖ ballots_hash[32] ‖ names_hash[32] ‖ borsh_bytes(tally_body)`, where `‖` is byte concatenation. The `chain_id` field (v0.3.10 N4) sits between `contract_addr` and `election_id`. The `ballots_hash` field (v0.3.11 B6, DST-prefixed in v0.3.14) is **32 raw bytes** (no length prefix; fixed size) sitting between `election_id` and `names_hash`, computed as `SHA-256(DST_VERIFIED_RCV_BALLOTS_V1 ‖ u32_LE(included_count) ‖ for each entry in candidate-declaration order: Borsh(voter: String) ‖ Borsh(ciphertext: bytes))` where `DST_VERIFIED_RCV_BALLOTS_V1 = b"verified-rcv:ballots:v1"` (23 ASCII bytes, no length prefix on the DST itself — it occupies a fixed-byte preamble that is structurally inseparable from the rest of the preimage). The `names_hash` field (v0.3.14) is **32 raw bytes** (no length prefix; fixed size) sitting between `ballots_hash` and `tally_body`, computed as `SHA-256(DST_VERIFIED_RCV_NAMES_V1 ‖ u32_LE(candidates.len()) ‖ for i in 0..N: Borsh(candidate_names[i]))` in candidate-declaration order where `DST_VERIFIED_RCV_NAMES_V1 = b"verified-rcv:names:v1"` (21 ASCII bytes, same discipline). The DSTs (v0.3.14 F2) prevent cross-hash substitution: even if `ballots_hash` and `names_hash` were ever exposed independently, their inner preimages disagree on the leading bytes, so neither can be substituted for the other. The contract reads `env.block.chain_id` + computes `ballots_hash` from `BALLOTS.range(...)` + computes `names_hash` from `election.candidate_names` at PublishResult time; the runtime computes both hashes from its received inputs. Divergence on any of `(ballots_hash, names_hash)` → `AttestationCommitMismatch` on chain.
- **Pairing binding** (v0.3.14 F4 clarification): the `(addr_i, name_i)` pair correspondence between `candidates` and `candidate_names` is bound *transitively* by the triple `(ballots_hash, names_hash, tally_body)`, not by a dedicated `candidates_hash` field. Each of the three commitments iterates in candidate-declaration order: `ballots_hash` walks `candidates` and binds the address ordering; `names_hash` walks `candidate_names` and binds the name ordering; `tally_body` carries addresses in `winners`, `per_round_counts`, `eliminated_by_round`, `dropped_voters`, `non_voters` — all keyed or ordered by candidate. The three together are injective over the pair set: any admin-supplied reordering or substitution of the (addr, name) pairs at CreateElection produces at least one differing hash at PublishResult time. A separate `candidates_hash` field is not needed because the existing triple already enforces the binding; documenting the transitivity explicitly closes the doc-gap that the F4 lens surfaced.
...
- **`data[0..32]`** carries a SHA-256 digest whose preimage depends on the quote purpose:
  - **Registration quote** (issued by the enclave at boot or pre-`CreateElection`): `data = SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id) ‖ names_hash)` where `enclave_pubkey` is the 33-byte compressed secp256k1 public key the enclave will use as the ECIES recipient identity, and `names_hash` is the 32-byte `compute_names_hash(election.candidate_names)` value. Cumulative provenance: v0.3.9 (`SHA-256(enclave_pubkey)`) → v0.3.12 N22 (added `Borsh(contract_addr) ‖ u64_LE(election_id)`) → v0.3.14 F1 (added `names_hash`). Binding via SHA-256 (collision-resistance hypothesis `Adv_pubkey_CR` carried as a degenerate case of `Adv_commitTally_CR`; same primitive, same hardness).
  - **Publish quote** (issued by the enclave at tally completion): `data = SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ ballots_hash ‖ names_hash ‖ tally_body))` — cumulative v0.3.8 M2 (`election_id` added), v0.3.10 N4 (`chain_id` added), v0.3.11 B6 (`ballots_hash` added), v0.3.14 F2 (`names_hash` added; DSTs introduced on internal hash preimages). Residence: `ReportData[0..32]` of the gnark public_inputs blob (was lower 32 bytes of `user_data` pre-v0.3.9).
...
- **Trigger:** `instantiate { candidates, candidate_names, start_at, end_at }` (v0.3.14: `candidate_names: Vec<String>` parameter added parallel to `candidates`)
- **Requires:**
  - `len(candidates) ≥ 1`
  - all candidate addresses distinct
  - `len(candidate_names) = len(candidates)` (v0.3.14)
  - each name is valid UTF-8, has byte-length ≥ 1 and ≤ 64, contains no NUL byte (v0.3.14)
  - all candidate names byte-distinct (no two equal) (v0.3.14)
...
- **Produces:**
  - stored: `candidates`, `candidate_names`, `start_at`, `end_at`, `enclave_pubkey`
  - `ballots = ∅`, `tally_result = None`
- **To state:** Created
- **On precondition failure:** `ContractError::InvalidInstantiation { reason }`; instantiation atomically reverts. v0.3.14 adds specific reasons: `CandidateNamesLengthMismatch`, `CandidateNameEmpty`, `CandidateNameTooLong`, `CandidateNameInvalidUtf8`, `CandidateNameContainsNul`, `DuplicateCandidateName`.
...
| B8 | attestation-binds-tally | **temporal** | `always (next.tally_result.is_some() ∧ tally_result.is_none() → the publish_result msg that fired carried (tally, proof, public_inputs) whose: (a) public_inputs has length 9_792 and conforms to the §2.5 gnark public_inputs byte layout (uints.U8 high-byte invariant + frontend.Variable u64-range invariant), (b) /xion.zk.v1.Query/ProofVerifyGnark returns verified=true when queried with (proof, public_inputs, registry.vkey_name), (c) ReportData[0..32] extracted from public_inputs equals SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ ballots_hash ‖ names_hash ‖ tally_body)) AND ReportData[32..64] equals DST_VERIFIED_RCV_TALLY_V1 zero-padded to 32 bytes, (d) the (MrTd, Rtmr1, Rtmr2) tuple extracted from public_inputs equals the on-chain registry's (mrtd, rtmr1, rtmr2) — plus (Rtmr0, Rtmr3) component-wise equality where the registry has bound those slots, (e) [registration-time companion, see B8(e) row below] the enclave_pubkey field of the active election was bound at CreateElection time by the same gnark verification primitive against a registration-purpose ReportData with data[0..32] = SHA-256(enclave_pubkey))`. **v0.3.9 reformulation (audit re-review N1)**: clauses (a) through (e) are now all **chain-verified-in-contract** — every publish exercises the gnark verifier directly via /xion.zk.v1.Query/ProofVerifyGnark and the contract performs bytewise equality on extracted public_inputs fields. Trust boundary narrowed: clauses (a) and (b) move from "inherited from Quartz tdxVerifier + groth16Verifier axioms (deferred-axiom)" to "inherited from Xion's xion.zk module ProofVerifyGnark semantics (chain-trust, §6.1)"; clauses (c) and (d) move from "verified-rcv-specific deployment binding via SHA-256(canonical_serialization) over 64-byte user_data" to "bytewise extraction from gnark public_inputs (no SHA-256 indirection on the measurements / DST; SHA-256 is still load-bearing on commit_hash in clause (c) via Adv_commitTally_CR in B9). The 32-byte commit_hash occupies ReportData[0..32] (was: lower 32 bytes of user_data); the 32-byte DST tag occupies ReportData[32..64] (was: upper 32 bytes of user_data). The pre-v0.3.9 user_data construct is removed. Collision-resistance hypothesis appears in B9 as Adv_commitTally_CR (unchanged primitive: SHA-256 over canonical_serialization). **Classical-Prop shadow note** (unchanged from prior version): B8 is stated as a deterministic implication; the cryptographic content lives in B9's negligibility budget. Consumer-facing claims in Section 6.6 ("Output contract") that read B8 as a categorical implication should be understood as classical-Prop shadows of the probabilistic B9 form (Attack 16). |
| B8(e) | pubkey-provenance | **temporal** | `always (next.elections[id].is_some() ∧ elections[id].is_none() → the create_election msg that fired carried (enclave_pubkey, proof, public_inputs) whose: public_inputs verifies via /xion.zk.v1.Query/ProofVerifyGnark against registry.vkey_name AND ReportData[0..32] extracted from public_inputs equals SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id) ‖ names_hash) AND ReportData[32..64] equals DST_VERIFIED_RCV_PUBKEY_V1 zero-padded to 32 bytes AND extracted measurements match registry)`. **Added v0.3.9 (audit re-review N1.c — pubkey provenance)**, **strengthened v0.3.12 N22 (registration replay defense)** to bind `(contract_addr, election_id)`, **strengthened v0.3.14 F1 (cross-family intent-adversarial fan-out)** to bind `names_hash`. The v0.3.8 form of CreateElection accepted enclave_pubkey from admin without cryptographic provenance; v0.3.9 closed that with the SHA-256(enclave_pubkey) binding; v0.3.12 closed cross-election replay; v0.3.14 closes the registration-not-bound-to-names asymmetry (defense-in-depth: tamper with names between CreateElection and PublishResult is now detectable at CreateElection-time, not only at publish-time). The cryptographic primitive is the same SHA-256 (collision-resistance bound by `Adv_commitTally_CR` in B9). Domain-separation tag `DST_VERIFIED_RCV_PUBKEY_V1` prevents cross-purpose replay between registration and publish quotes (same circuit + vkey, different ReportData purpose). Residual risk: admin running a different VM whose MRTD matches the registry — narrower governance surface (N2 timelock + multisig admin operational mitigation). |
...
| B11 | candidate_names immutable post-CreateElection | **temporal** | `always (∀ id ∈ ELECTION_COUNTER-domain, elections[id].candidate_names ≠ ⊥ → next.elections[id].candidate_names = elections[id].candidate_names)` — once an election's `candidate_names` field is written by CreateElection (or by `HISTORICAL_ELECTIONS` archival in v0.3.12), the value is never mutated. **Added v0.3.14**: parallels B1's monotone-once-set discipline applied to `candidate_names`. No handler mutates the field after CreateElection; the chain-side enforcement is structural (CreateElection is the sole site that *assigns* `Election.candidate_names`; other handlers that re-save the `Election` struct preserve the field byte-identically, e.g. `exec_submit_ballot` increments `ballot_count` and writes the whole struct back without touching `candidate_names`). No `UpdateNames` handler exists. The publish-time `names_hash` in `canonical_serialization` cross-checks the value the runtime saw against the chain's stored value — a host that mutates names mid-flight produces a mismatched hash and the publish attestation rejects with `AttestationCommitMismatch`. Discipline lens AB.6 (deferral-justification audit): no deferral; the immutability is structurally enforced, not deferred. |
...
#### Discharge annotations (v0.3.13 bootstrap per v0.4 asks AB.2 + AC)

For each named B-clause: a `code:` field pointing to the discharging file:line, plus (where the discharge is chain-side and an existing or pending Kani harness covers it) a `kani:` field. Off-chain / upstream / axiom obligations are tagged explicitly with the reason. Paths are repo-relative. Annotations are validated against HEAD and updated when discharge sites move.

- **B1 — tally_result monotone-once-set**
  - `code: crates/contract/src/contract.rs:419-420` — `exec_publish_result` returns `AlreadyResolved` when `phase == Phase::Resolved`, ensuring `TALLY_RESULT` is written at most once. Pair with `crates/contract/src/contract.rs:453` (`TALLY_RESULT.save`, the unique write site) and `crates/contract/src/state.rs:146` (the `Item<TallyResult>` storage declaration).
  - `kani: already_resolved_enforced` at `crates/contract/src/verification.rs:400` (rejects second publish) and `b1_tally_result_present_after_publish` at `crates/contract/src/verification.rs:217` (post-publish presence).

- **B6 — ballot writer is the ballot voter**
  - `code: crates/contract/src/contract.rs:379-385` — `exec_submit_ballot` rejects `VoterNotCandidate` when `info.sender ∉ candidates`, rejects `AlreadyVoted` on repeat, and writes `BALLOTS.save(deps.storage, &info.sender, &ciphertext)` so the storage key is bound to the message sender by construction.
  - `kani: already_voted_enforced` at `crates/contract/src/verification.rs:358` (no double-write under last-write-wins absent revision intent) and `s4_ballot_keys_subset_of_candidates` at `crates/contract/src/verification.rs:259` (the structural complement: every ballot key is a candidate).

- **B8(a) — gnark `public_inputs` shape conforms to §2.5 layout**
  - `code: crates/contract/src/contract.rs:756` — `validate_public_inputs_shape` enforces length == 9_792 + the `uints.U8` high-byte invariant + the `frontend.Variable` u64-range invariant; called from `verify_publish_quote` at `crates/contract/src/contract.rs:901` and `verify_registration_quote` at `crates/contract/src/contract.rs:931`.
  - `kani: skipped because shape-validation is a bounded byte-length + per-element range check; covered by 12+ contract-level negative tests in crates/contract/src/contract.rs::tests (e.g., wrong-length, u8-out-of-range, u64-out-of-range). A symbolic Kani harness over the full 9_792-byte input space exceeds the project's per-harness state-space budget; the negative tests + the deterministic byte-level invariants make Kani redundant here. Tracked as candidate for future Kani uplift per v0.4 ask AC.`

- **B8(b) — `/xion.zk.v1.Query/ProofVerifyGnark` returns `verified=true`**
  - `code: crates/contract/src/contract.rs:845` — `verify_gnark_proof_via_xion` (production cfg) issues the gnark stargate query and returns `AttestationGnarkVerifyFailed` on any non-verified response; called from `verify_publish_quote` at `crates/contract/src/contract.rs:913` and `verify_registration_quote` at `crates/contract/src/contract.rs:967`. Under the `mock-attestation` cfg the alternate stub at `crates/contract/src/contract.rs:880` makes the chain-side delegation a no-op for unit-test runs; production builds compile out the stub per v0.3.10 N5 CI grep.
  - `kani: skipped because the gnark verifier is an upstream chain primitive (xion.zk module). Kani cannot model the stargate boundary. Discharged at the chain-trust layer (§6.1) and at the substrate axiom layer via Quartz Zkdcap.groth16Verifier (§6.2). CI's wasm symbol-grep at .github/workflows/ci.yml (v0.3.10 N5) ensures the production binary does not retain the mock stub.`

- **B8(c) — `ReportData[0..32] == SHA-256(canonical_serialization(...))` + DST tag in `[32..64]`**
  - `code: crates/contract/src/contract.rs:907` — `verify_publish_quote` compares `rd[..32]` to the chain-computed `expected_commit` (built at `crates/contract/src/contract.rs:438` via `compute_commit_hash`); DST tag enforced at `crates/contract/src/contract.rs:911` via `check_dst_tag(&rd, DST_TALLY_LITERAL)` (impl at `crates/contract/src/contract.rs:821`). `compute_commit_hash` impl at `crates/contract/src/contract.rs:981`; `canonical_serialization` at `crates/contract/src/contract.rs:999`. Runtime mirror at `crates/enclave/src/attestation.rs:72`. Byte-identical cross-test `canonical_serialization_byte_for_byte` at `crates/enclave/src/attestation.rs:718`.
  - `kani: skipped because the discharge is a SHA-256 equality check between two byte-vector computations whose byte-identity is asserted by deterministic cross-tests in crates/enclave/src/attestation.rs::tests (canonical_serialization_byte_for_byte, ballots_hash_affects_canonical_serialization_bytes, chain_id_affects_canonical_serialization_bytes, dst_tags_match_intent_v0_3_9). Collision-resistance is the cryptographic content (bounded by Adv_commitTally_CR in B9), not a structural property Kani can reason about. Tracked as candidate for a property-test layer uplift, not Kani.`

- **B8(d) — `(MrTd, Rtmr1, Rtmr2[, Rtmr0, Rtmr3])` extracted from `public_inputs` equals on-chain registry**
  - `code: crates/contract/src/contract.rs:769` — `verify_measurements_match_registry` performs component-wise equality (mandatory: mrtd, rtmr1, rtmr2; optional: rtmr0, rtmr3 only when the registry binds those slots); extraction helpers at `crates/contract/src/contract.rs:733` (`extract_measurement_48`); registry shape at `crates/contract/src/state.rs:106` (`EnclaveImageRegistry`); registry well-formedness validation at `crates/contract/src/contract.rs:1141` (`validate_registry`).
  - `kani: skipped because the discharge is bytewise field equality between fixed-offset slices of public_inputs and registry fields. Eight contract-level negative tests in crates/contract/src/contract.rs::tests cover each variant (mrtd mismatch, rtmr1 mismatch, rtmr2 mismatch, optional rtmr0 bound-and-mismatch, optional rtmr3 bound-and-mismatch, optional rtmr0 unbound passes, registry shape 47-byte-mrtd rejected, registry shape 49-byte-mrtd rejected). Kani over the 1536-byte MrTd field exceeds the state-space budget. Tracked as candidate for AC uplift; current discharge is by exhaustive negative tests plus the deterministic slice extraction.`

- **B8(e) — registration-time pubkey-provenance binding**
  - `code: crates/contract/src/contract.rs:921` — `verify_registration_quote` enforces `rd[..32] == SHA-256(enclave_pubkey ‖ Borsh(contract_addr) ‖ u64_LE(election_id))` (the v0.3.12 N22 three-axis binding) at `crates/contract/src/contract.rs:944`, DST tag at `crates/contract/src/contract.rs:965`, gnark verification at `crates/contract/src/contract.rs:967`. Called from `exec_create_election` at `crates/contract/src/contract.rs:326`. Preimage builder `build_registration_report_data` at `crates/contract/src/contract.rs:1299` (chain) and `crates/enclave/src/attestation.rs:209` (runtime); election_id/contract_addr binding cross-tests at `crates/enclave/src/attestation.rs:671` (`registration_report_data_election_id_binds_n22`) and `crates/enclave/src/attestation.rs:681` (`registration_report_data_contract_addr_binds_n22`).
  - `kani: skipped because the discharge is the same SHA-256 + bytewise extraction pattern as B8(c) but at registration time; covered by contract-level negative tests for the v0.3.12 N22 replay-across-elections rejection path (RegistrationQuoteWrongElection diagnostic, AttestationPubkeyBindingMismatch fallback). Tracked as candidate for AC uplift.`

- **B9 — B8 negligibility-budget decomposition (meta-security)**
  - `code: axiom — meta-security bound, not discharged by chain-side code. Decomposes into three cryptographic-advantage summands: (i) Adv_tdxVerifier_sound bounded by Quartz Specs.Quartz.Attestation.Dstack.tdxVerifier (Quartz axiom, see §6.2 inheritance row 1; consumed via B8(d) measurement extraction at crates/contract/src/contract.rs:769); (ii) Adv_groth16_KS bounded by Quartz Specs.Quartz.Attestation.Zkdcap.groth16Verifier (Quartz axiom, see §6.2 inheritance row 2; consumed via B8(b) gnark verification at crates/contract/src/contract.rs:845) plus the circuit_equivalence_honest precondition on the registered vkey_name (operational precondition, no chain-side code); (iii) Adv_commitTally_CR bounded by SHA-256 collision-resistance over canonical_serialization (verified-rcv-internal; the SHA-256 primitive is invoked at crates/contract/src/contract.rs:990-992 within compute_commit_hash, but its collision-resistance is the standard cryptographic assumption, not a property the discharging code asserts). Conditioned on image_registration_honest(σ) per §6.1.`
  - `kani: skipped because meta-security probabilistic bounds are not expressible as Kani assertions. The three primitives bounding the summands (TDX quote validity, Groth16-KS soundness, SHA-256 collision-resistance) are upstream cryptographic axioms.`

- **B10 — tally-correctness (cross-layer composition)**
  - `code: off-chain — composition theorem with no single chain-side discharge site. The 5-link composition is recorded in §8.7 (B10 ← B10_lean ∧ image-identity-binding ∧ B8 ∧ dstack_kms_trust ∧ enclave_input_fidelity) and assembled in the integration ledger. The contract-side B8 footprint is at crates/contract/src/contract.rs:445 (verify_publish_quote call) + the per-link citations under §8.7 below.`
  - `kani: skipped because B10 is cross-layer (Lean theorem + build-pipeline binding + chain-side B8 + operational KMS trust + cross-layer input-fidelity). No single chain harness can witness it. The chain-side projection inv_b10_chain_side_projection at specs/rcv.qnt:510 is the Quint-level companion; the Lean discharge half lives in specs/EnclaveBridge.lean:163 (B10_lean_irv, currently sorry-bodied).`

- **B10_lean — Lean-internal image-IO obligation**
  - `code: specs/RcvSpec.lean:1487` — `theorem B10_lean : ∀ raw cs pk, EnclaveImage raw cs pk = Tally_spec raw cs pk`, body `sorry`. The discharge path is via `specs/EnclaveBridge.lean:163` (`theorem B10_lean_irv`, also `sorry`-bodied) composed with the Stage-1 placeholder axiom `specs/EnclaveBridge.lean:182` (`axiom B10_lean_decrypt`).
  - `kani: skipped because B10_lean is off-chain by tag (§3.2 row tag = off-chain) — a pure Lean theorem about an extracted model. No chain-state witness exists by construction.`

- **B11 — candidate_names immutable post-CreateElection** (v0.3.14)
  - `code: crates/contract/src/contract.rs::exec_create_election` (sole writer of `Election.candidate_names`; field set during CreateElection and no subsequent handler mutates it). Storage shape pinned in `crates/contract/src/state.rs::Election`. Archival to `HISTORICAL_ELECTIONS` preserves the field byte-identically. The publish-time `names_hash` in `canonical_serialization` (via `compute_names_hash`) cross-checks runtime-supplied names against stored names — any tamper triggers `AttestationCommitMismatch` at the chain layer.
  - `kani: names_mismatch_rejected` — symbolic byte-flip in the names_hash slot of public_inputs asserts `AttestationCommitMismatch`. Catalog entry per AC top-down discipline.
...
- **Off-chain responsibility — candidate names** (v0.3.14): the instantiator also picks the display names parallel to `candidates`. The chain enforces structural well-formedness (length match, UTF-8, ≤ 64 bytes, no duplicates, no NUL) and chain-side immutability (B11), but it does NOT verify that name "Alice" actually labels the IRL Alice. A malicious instantiator who lists candidate addresses correctly but mislabels them (e.g., names address A "Bob" and address B "Alice") deceives voters who rely on names rather than addresses. The chain-side defense is partial: the `names_hash` binding in `canonical_serialization` ensures that the names voters saw at vote-time and the names attested at publish-time are byte-identical (otherwise the publish attestation rejects). Names cannot be silently swapped post-publish. **Names are off-chain trust** in the same sense addresses are; the chain provides byte-identity over time but not real-world-identity correspondence. Voters who want stronger guarantees must verify name-to-address correspondence out of band (e.g., signed declarations by the named candidates, public commitments before instantiation).
- **Off-chain responsibility — visual-confusability of names** (v0.3.14, surfaced by AB.4 who-controls lens during the intent-adversarial pass): the chain's duplicate-name check is byte-equality. It rejects two literally-equal strings but accepts visually-confusable pairs such as Cyrillic "Аlice" (U+0410) and Latin "Alice" (U+0041), or homoglyph attacks with whitespace / combining marks / RTL overrides. A malicious instantiator can present visually-identical names mapped to different addresses; voters relying purely on visual identity may vote for the wrong candidate. The chain provides byte-distinctness; visual-distinctness is the instantiator's off-chain responsibility. UIs SHOULD warn on confusable-character detection (NFC normalization + Unicode confusables-table check), but the chain does not. Treated as scope-bounded analogously to candidate-address legitimacy: the methodology stance is that voters with stronger requirements verify out of band.
...
### 6.7 Contract migration discipline (v0.3.14 F3)

verified-rcv ships as a CosmWasm contract. CosmWasm supports contract migration via `MigrateMsg`; verified-rcv currently does not expose a migration handler, but a future revision might. This subsection pins the migration trust-boundary obligations that downstream code MUST preserve.

**B11 structural-enforcement invariant.** `Election.candidate_names` is immutable post-CreateElection by virtue of the contract having no `UpdateNames` (or `SetNames`, or `EditElection`) handler. Any future contract revision that introduces a handler whose execution writes to `Election.candidate_names` (or that loads + modifies + saves the field in any code path other than `exec_create_election`) constitutes a **behavioral break** under the v0.3.0 SemVer rubric and requires a MAJOR version bump plus a fresh trust-boundary audit. The discipline is enforced structurally (absence of code), not via a runtime lock-flag, because a determined attacker who can change handlers can also clear a lock-flag — the lock-flag is theatre, the missing code is the real defense.

**Per-clause migration table.** For each named B-clause + §8.7 link, the migration discipline is:

| Invariant | Migration discipline |
|---|---|
| B1 (tally_result monotone-once-set) | TALLY_RESULT writes only via `exec_publish_result` with phase-gate; any new handler writing TALLY_RESULT is a MAJOR break. |
| B6 (ballot writer = ballot voter) | BALLOTS writes only via `exec_submit_ballot` with `msg.sender ∈ candidates` check; bypassing the check via a new handler is MAJOR. |
| B8(a-e) (attestation binds tally) | Any handler that publishes TALLY_RESULT without `verify_publish_quote` discharge is MAJOR. |
| B11 (candidate_names immutable) | Any handler *mutating* `Election.candidate_names` post-create is MAJOR. (Handlers that re-save the `Election` struct preserving the field byte-identically — e.g., `exec_submit_ballot` updating `ballot_count` — are compliant.) |
| §8.7 link 4 (image-identity-binding) | Any change to the registered MRTD/RTMR semantics in the registry must follow the v0.3.10 timelock + N17 dual-gate flow; bypassing is MAJOR. |
| §8.7 link 7 (enclave_input_fidelity) | Any change to `canonical_serialization` byte layout that does not preserve injectivity over (chain_id, contract_addr, election_id, ballots, names, tally) is MAJOR. |

**Discharge annotation.** The migration discipline is encoded in `crates/contract/src/contract.rs` by what handler functions exist (and what they touch); future audits MUST diff the handler set + their storage writes against this table. A `colosseum-change` skill invocation (per Round 3f composition discipline) is the appropriate trigger when any of these surfaces is touched.
...
#### Per-link discharge annotations (v0.3.13 bootstrap per v0.4 asks AC + AD)

Each link in the composition above carries two annotations: `code: file:line` (the discharging code site or an explicit `off-chain`/`axiom`/`upstream` reason) and `kani: <harness>` (the Kani harness witnessing it, or an explicit skip with reason). Paths repo-relative; line numbers anchored to HEAD at v0.3.13 cut.

1. **Stage-1 lemma — `decrypt_and_validate` correctness**
   - `code: off-chain — Lean Stage-1 obligation. Current Lean discharge is the axiom placeholder specs/EnclaveBridge.lean:182 (axiom B10_lean_decrypt) because the Aeneas-extracted Stage-1 body in crates/enclave-core is currently unimplemented!() (Stage 1 lives in the runtime crate which uses ECIES + dstack; extraction queued for a future round). The runtime-side decryption code lives at crates/enclave/src/dstack.rs (ECIES decoder) and the contract has no Stage-1 discharge by design (Stage 1 runs inside the enclave).`
   - `kani: skipped because Stage 1 is off-chain (ECIES decryption + permutation validation inside the TDX enclave). The chain has no Stage-1 invariant to assert; Kani is a chain-side tool.`

2. **Stage-2 theorem — `IRV_spec` correctness**
   - `code: specs/RcvSpec.lean:1128 (theorem irv_ballots_tallied), specs/RcvSpec.lean:1292 (theorem irv_winners_shape), specs/RcvSpec.lean:1323 (theorem irv_round_counts_sum), specs/RcvSpec.lean:1362 (theorem irv_no_reappearance) — concretized math IRV_spec at specs/RcvSpec.lean:237 (def IRV_spec). Three Stage-2 obligation theorems are sorry-bodied awaiting structural induction on irv_loop fuel per Round 3e (CLAUDE.md project state).`
   - `kani: skipped because IRV_spec correctness is a Lean theorem about a recursive math function (off-chain). The chain-side well-formedness checks that exercise Stage-2 outputs are crates/contract/src/verification.rs:542 (s6_non_candidate_winner_rejected), :582 (s7_partition_equation_required), :620 (s8_round_sum_required), :661 (s9_reappearance_rejected) — these witness the structural shape Stage 2 produces, but the correctness-of-the-recursion proof itself is the Lean theorem.`

3. **Lean-internal composition — `B10_lean`**
   - `code: specs/RcvSpec.lean:1487 (theorem B10_lean : ∀ raw cs pk, EnclaveImage raw cs pk = Tally_spec raw cs pk, body sorry); composition through the extracted-model bridge at specs/EnclaveBridge.lean:163 (theorem B10_lean_irv, body sorry).`
   - `kani: skipped because B10_lean is off-chain by tag (§3.2 row tag = off-chain). A Lean theorem about an extracted model has no chain-state witness.`

4. **Image identity binding (extraction-soundness + build-reproducibility)**
   - `code: off-chain — build pipeline + deployment ledger obligation, no chain-side code line. The chain consumes the binding indirectly via crates/contract/src/state.rs:106 (EnclaveImageRegistry holds the registered MrTd/Rtmr*) and crates/contract/src/contract.rs:769 (verify_measurements_match_registry checks public_inputs against registry). The Lean-extraction-soundness sub-obligation is tracked in the project ledger; the reproducible-build sub-obligation depends on the dstack image hash + Lean Aeneas extraction provenance recorded under .colosseum/ledger.md.`
   - `kani: skipped because the binding is between a binary artifact and a Lean-extracted model — neither side is a chain trajectory. Kani cannot model the off-chain build pipeline.`

5. **B8 (chain-side attestation verification)**
   - `code: crates/contract/src/contract.rs:445 (verify_publish_quote call site inside exec_publish_result), discharged at crates/contract/src/contract.rs:893 (pub fn verify_publish_quote — chains validate_public_inputs_shape + verify_measurements_match_registry + verify_tcb_status_accepted + commit-hash equality + DST tag + verify_gnark_proof_via_xion). Companion registration-quote discharge at crates/contract/src/contract.rs:921 (verify_registration_quote), called from exec_create_election at crates/contract/src/contract.rs:326.`
   - `kani: skipped because the chain-side B8 check is a composite of (a) bounded byte-range validation over a 9_792-byte public_inputs vector (Kani state-space blowup, tracked under v0.4 ask AC); (b) bytewise equality on 32/48-byte slices (covered by exhaustive contract-level negative tests in crates/contract/src/contract.rs::tests — at least 12 cases for shape, 8 for measurement, multiple for ReportData / DST / TCB); (c) the gnark verifier itself (upstream xion.zk module, not Kani-modellable per B8(b) annotation above). Tracked as candidate for AC top-down Kani uplift; a non-symbolic harness exercising verify_publish_quote with kani::any() over the registry + a fixed-shape public_inputs structure could plausibly fit within the per-harness state-space budget.`

6. **`dstack_kms_trust` (operational link)**
   - `code: axiom — operational assumption on dstack KMS implementation honesty per §6.3 trust boundary. No chain-side code line discharges this; the chain has no read access to the KMS derivation discipline. The runtime obtains the per-election privkey via crates/enclave/src/dstack.rs (KMS client) but the trust that the returned privkey is the KMS-derived key for the registered (MrTd, RTMR) tuple is upstream from verified-rcv. Same trust surface as failure mode 4.2 (KMS key compromise — confidentiality face) viewed from its integrity-of-keying-discipline face.`
   - `kani: skipped because dstack_kms_trust is an axiom on an off-chain operational layer (the KMS daemon). No chain-side invariant to assert; no Kani-modellable boundary.`

7. **`enclave_input_fidelity` — discharged v0.3.11 B6**
   - `code: crates/contract/src/contract.rs:437 (chain computes ballots_hash = compute_ballots_hash(&election.candidates, &ballots_view) at PublishResult time); chain-side compute_ballots_hash impl at crates/contract/src/contract.rs:1031; runtime-side mirror at crates/enclave/src/attestation.rs:97. Both feed into the commit hash at crates/contract/src/contract.rs:438 (compute_commit_hash) which lands in ReportData[0..32] verified at crates/contract/src/contract.rs:907. A host that substitutes raw_ballots produces a runtime ballots_hash diverging from the chain's, triggering AttestationCommitMismatch. **v0.3.14 extension**: names_hash also enters the commit_hash preimage via compute_names_hash(&election.candidate_names) (chain side) and compute_names_hash(&candidate_names) (runtime side, from the TallyRequest); divergence on names_hash likewise triggers AttestationCommitMismatch. Cross-test compute_names_hash_contract_vs_runtime_byte_identical asserts byte-equality. Link 7 now reads as input-fidelity over the (ballots, candidate_names) pair rather than ballots alone.`
   - `kani: names_mismatch_rejected at crates/contract/src/verification.rs (v0.3.14) — symbolically flips one byte (k < 32) of the chain-side names_hash leg of the commit_hash preimage and asserts verify_publish_quote rejects with AttestationCommitMismatch. Witnesses the v0.3.14 extension of link 7 over the names_hash component. The ballots_hash leg face remains skipped at the Kani layer because the discharge is byte-identity between two independent ballots_hash implementations (chain + runtime) plus SHA-256 collision-resistance: byte-identity is asserted by cross-test ballots_hash_candidate_declaration_order_pinned at crates/enclave/src/attestation.rs:629 and the broader canonical_serialization byte-identity tests cited under B8(c); collision-resistance is bounded by Adv_commitTally_CR in B9 (cryptographic axiom). Neither face of the ballots_hash leg is Kani-expressible.`

8. **Composition — B10**
   - `code: off-chain — pure composition theorem combining links 1+2+3 (B10_lean Lean discharge) + link 4 (image-identity-binding, build-pipeline) + link 5 (B8 chain-side) + link 6 (dstack_kms_trust axiom) + link 7 (enclave_input_fidelity, ballots_hash chain witness). Assembled in the integration ledger at .colosseum/ledger.md (v0.3.13 cut). No single code site; the contract-side footprint is the per-link citations above.`
   - `kani: skipped because B10 is cross-layer (Lean + build pipeline + chain attestation + operational KMS trust + cross-layer input-fidelity) per §3.2 row tag. The chain-side projection is inv_b10_chain_side_projection at specs/rcv.qnt:510 (Quint companion); no Kani harness can witness the full composition.`
