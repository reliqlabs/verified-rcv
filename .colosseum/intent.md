...
**Document version: 0.3.10** (SemVer per Colosseum methodology v0.4 candidate Ask N — see `colosseum/methodology-v0.4-candidates.md`). Version-bump classification:
...
**Canonical serialization** (added per Round 3a re-adversarial pass — themes 6+13, mistral A10 / qwen A4 / kimi A10 / kimi A7; **leaf-encoding pins added per v0.3.1 Round 3a subagent-dispatch synthesis T7** — 3 voices flagged the prior version as under-specified at the `Addr` / `Nat` level; **`election_id` added v0.3.8 M2; `chain_id` added v0.3.10 N4 for cross-chain replay defense**): every appearance of `canonical_serialization(x)` in this doc (most notably B8 clause (c) and Block E1's attestation construction) refers to **Borsh** serialization per the CosmWasm-native discipline. Concretely:
- `canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ tally_body)` denotes `borsh_bytes(contract_addr) ‖ borsh_bytes(chain_id) ‖ u64_LE(election_id) ‖ borsh_bytes(tally_body)`, where `‖` is byte concatenation. The `chain_id` field (v0.3.10 N4) sits between `contract_addr` and `election_id`; the contract reads `env.block.chain_id` at PublishResult time and passes it to the commit-hash computation.
...
- **`data[0..32]`** carries a SHA-256 digest whose preimage depends on the quote purpose:
  ...
  - **Publish quote** (issued by the enclave at tally completion): `data = SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ tally_body))` per v0.3.8 M2 — unchanged from v0.3.8 except for residence: was lower 32 bytes of `user_data`; is now `ReportData[0..32]` in the gnark public_inputs blob.
...
- **Requires:**
  ...
  - `ReportData[0..32]` extracted from `public_inputs` equals `SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ tally_body))` — see B8 clause (c)
...
- **Produces:** (equivalently: compute `Tally_spec(ballots, candidates, enclave_privkey)` per Section 2.5, then attest + submit)
  ...
  4. **Request TDX quote** from `tappd` (dstack's Trusted App Daemon) with `report_data = SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ tally_body)) ‖ DST_VERIFIED_RCV_TALLY_V1_PADDED` (per §2.5 ReportData layout — publish quote form).
...
7. The enclave requests a publish-purpose TDX quote whose `report_data = SHA-256(canonical_serialization(contract_addr ‖ chain_id ‖ election_id ‖ tally_body)) ‖ DST_VERIFIED_RCV_TALLY_V1_PADDED`, drives the zkdcap gnark prover to produce `(proof, public_inputs)`, and calls `publish_result { tally, proof, public_inputs }`. The contract queries `/xion.zk.v1.Query/ProofVerifyGnark`, extracts and compares measurements + ReportData per §2.5 layout, checks tally well-formedness, and sets `tally_result = Some(tally)`. Contract enters Resolved state.
...
7. **`enclave_input_fidelity` (cross-layer link, added v0.3.1 T3)**: the input the enclave actually decrypted equals `ballots@end_at` (the chain-side frozen ballot set, per B2 + §2.5 Stage-1 input discipline) plus the publicly-readable `candidates` and time fields. This conjunct rules out a malicious-host-substituted input set (the host feeds the enclave a different `raw_ballots` than what is on chain). Discharge path: the publish-purpose ReportData (§2.5 ReportData layout, v0.3.9) binds the *output* (commit_hash over `canonical_serialization(contract_addr ‖ election_id ‖ tally_body)`) but NOT the *input* (raw_ballots). The input-fidelity conjunct therefore additionally requires either (i) the publish ReportData to also bind `SHA-256(Borsh(raw_ballots_chainview))` — e.g., by extending `canonical_serialization` to include `ballots_hash`, or by repurposing 32 of the 64 ReportData bytes — or (ii) a chain-inclusion-proof obligation showing the enclave's input equals the chain's `ballots@end_at` snapshot. **(Still pending as of v0.3.9 — choice between (i) and (ii) is deferred. v0.3.9 narrowed B8(a)/(b)/(d) but did not touch the input-fidelity discharge. Option (i) is cheap once chosen: extend `canonical_serialization` per the v0.3.8 M2 pattern with a `ballots_hash` field after `election_id`, and the existing ReportData binding closes link 7. Tracked as the remaining open methodology obligation.)**
