...
**Document version: 0.3.8** (SemVer per Colosseum methodology v0.4 candidate Ask N — see `colosseum/methodology-v0.4-candidates.md`). Version-bump classification:
...
| `verifyGroth16` accepts a proof iff it is a valid Groth16 proof of the encoded TDX-quote-validity statement under the registered vkey | `Specs.Quartz.Attestation.Zkdcap` (`groth16Verifier` record); honest formulation at `proofs/lean/Specs/Quartz/Attestation/ZkdcapVCVio.lean:98-105` | (d) classically-over-strong-doubled-negligibility | ArkLib Groth16 KS reduction + reference DCAP circuit-equivalence theorem. **Bundling note (v0.3.7 from Quartz audit 2026-05-26)**: Quartz's `groth16Verifier` lift is "doubled" — it bundles `negligible_groth16` (Groth16-KS on BN254) AND `negligible_circuit` (R1CS encoding ↔ reference DCAP verifier semantic equivalence). verified-rcv's §3.2 B9 retag (v0.3.1) split this doubled lift into `Adv_groth16_KS` (the cryptographic-hardness summand) plus `circuit_equivalence_honest` (a *binary correctness precondition*, lifted out of the negligibility sum per the v0.3.1 T9 / kimi temporal-invariants attack 7 finding that circuit equivalence is a universal `∀ inputs, R1CS(I) = reference(I)` property and not a probabilistic gradient). The two framings are semantically equivalent under degenerate-negligibility interpretation (`negligible_circuit` is 0 if encoding is correct, 1 if not — binary), but verified-rcv's framing makes the load-bearing claim inspectable as a Lean theorem rather than buried in a `negligible_circuit` summand whose proof is "the encoding is correct, q.e.d." |
