...
| 5 | `irv_round_counts_sum` | Lean **theorem** (DISCHARGED 2026-05-26) | (b) demotable-to-derived-theorem | `theorem-discharged` | S8 | Records that each round's per-candidate counts sum to `ballots_tallied` — the conservation property of IRV. **DISCHARGED via structural induction on `irv_loop` fuel** + the strong S8 helper `tally_round_total_count_eq_valid_length`. Required 8 additional auxiliary lemmas (count_at_index_cons, tally_round_aux_total_count_cons, total_count_tally_round_aux_nil, total_count_tally_round_nil, tally_round_total_count_eq_valid_length, total_count_zero_round, foldl_count_add_eq, foldl_count_eq_total_count, plus the loop invariant irv_loop_per_round_total). Carries `cs.Nodup` hypothesis (proof needs it via candidates_with_count_nodup chain — same A4-style Nodup-propagation pattern as `irv_winners_shape`). Axiom-clean: `propext` + `Quot.sound` only. |
| 6 | `irv_no_reappearance` | Lean **theorem** (DISCHARGED 2026-05-26) | (b) demotable-to-derived-theorem | `theorem-discharged` | S9 | Records elimination monotonicity: a candidate eliminated at round `i` does not reappear in any later `per_round_counts[j]?` with `j > i`. **DISCHARGED via structural induction on `irv_loop` fuel** with case-split on `valid = []` (vacuous via empty eliminated_by_round) vs `valid ≠ []` (uses cover + the all-abstain-branch-unreachable property + the per_round_counts-subset-of-remaining invariant + remove_from_not_mem). Required 2 additional auxiliary lemmas (remove_from_not_mem, irv_loop_per_round_subset_remaining) plus the loop invariant irv_loop_no_reappearance. Carries `cs.Nodup` hypothesis. Axiom-cleanliness: `propext` + `Classical.choice` + `Quot.sound` (Classical.choice from `Classical.byContradiction` used in the recursive-case length-bound argument). |
...
**0 hits**. Each of the 9 trust-surface items above is referenced by at least one composition theorem or downstream Lean theorem. Round 3e (completing 2026-05-26) converted #2, #4, #5, #6 from `axiom` to fully-DISCHARGED Lean theorems against the concrete math `IRV_spec`. #3 stays `axiom` because Stage 1's concrete body lives in the not-yet-extracted runtime crate `verified-rcv-enclave`.

## Per-tool coverage snapshot

| Tool | Artifacts | Proven / Verified | Outstanding |
|------|-----------|-------------------|-------------|
| Lean math spec (`specs/RcvSpec.lean`, Lake project with Mathlib 4.30.0-rc2 + Aeneas) | 5 theorems (S6-S9 + B10_lean) + 4 obligation theorems (all DISCHARGED) + ~22 auxiliary lemmas + 2 axioms (decrypt_partition_length + EnclaveImage) + 1 opaque (decrypt_and_validate) + **concrete `IRV_spec`** (Round 3e) | **8 theorems discharged** (S6, S7, S8, S9 + 4 Stage-2 obligations including irv_winners_shape + irv_round_counts_sum + irv_no_reappearance, all via structural induction on `irv_loop` fuel) | 1 `sorry` remaining: B10_lean (the image-IO obligation bridging to Aeneas-extracted enclave) |
...
| Rust enclave runtime (`crates/enclave/`) | Stage 1 `decrypt_and_validate` (ECIES + Borsh + permutation validation), `tally_spec` composition, gRPC `TallyService` (tonic + tally.proto), dstack KMS/quote client (simulator + real-TDX paths), Quartz attestation envelope construction (domain-tagged SHA-256 commit), `bin/server.rs` end-to-end against `DSTACK_SIMULATOR=1` | **17/17 tests pass** (Stage 1 partition behaviors + tally composition + envelope binding + dstack simulator + gRPC smoke) | Real-TDX path needs hardware + `ZKDCAP_PROVER_URL` for zkdcap proof generation; TDX-quote parser to populate `mrtd_hex` / `rtmr_hex` in `HealthResponse` |
...
| (b) demotable-to-derived-theorem | 4 `theorem-discharged` (irv_ballots_tallied + irv_winners_shape + **irv_round_counts_sum DISCHARGED 2026-05-26** + **irv_no_reappearance DISCHARGED 2026-05-26**), 1 `derived-from-spec-model` (decrypt_partition_length) | 5 | unchanged in number; **#5 and #6 both promoted from theorem-pending to theorem-discharged** 2026-05-26 via the full S8/S9 chain (~20 auxiliary lemmas + 2 loop invariants) |
...
| **Total** | | **9** | **0** (Round 3e complete: 4 of 5 (b)-bucket entries fully discharged; 1 axiom-bodied (decrypt_partition_length, pending runtime-crate Aeneas extraction)) |

**Round 3e COMPLETE for the math layer**: all four Stage-2 obligation theorems (irv_ballots_tallied, irv_winners_shape, irv_round_counts_sum, irv_no_reappearance) are DISCHARGED against the concrete math `IRV_spec`. S6/S7/S8/S9 follow as Lean theorems against the concrete spec via `Tally_spec`. The (b) bucket has one remaining axiom (`decrypt_partition_length` — Stage 1 partition obligation), demotable once the runtime crate's Aeneas extraction lands. Round 3e progress: math `IRV_spec` concrete; the all-abstain-branch-unreachable insight (under A8 cover + non-empty valid) makes both S8 and S9 tractable.
...
5. **Rust enclave runtime crate** — **landed 2026-05-26**: Stage 1 (ECIES + Borsh + permutation validation), `tally_spec` composition, gRPC `TallyService`, simulator-backed dstack KMS/quote client, Quartz attestation envelope construction (domain-tagged SHA-256 commit) — 17/17 tests pass. Remaining: real dstack TDX boot (hardware required), zkdcap prover integration for the production-mode envelope, TDX-quote parser to populate `HealthResponse.mrtd_hex` / `.rtmr_hex`.
